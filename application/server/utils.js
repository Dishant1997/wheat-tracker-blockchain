...
utils.registerUser = async (userid, userpwd, usertype, userCnic, userCity, adminIdentity) => {
    console.log("\n------------  utils.registerUser ---------------");
    console.log("\n userid: " + userid + ", pwd: " + userpwd + ", usertype: " + usertype + ", userCnic: " + userCnic)
...
utils.getUser = async (userid, adminIdentity) => {
    ...
    if (userid == "admin") {
        ...
        while (user.result.attrs[i].name !== "userCnic") i++;
            result.userCnic = user.result.attrs[i].value;
        while (user.result.attrs[k].name !== "userCity") k++;
            result.userCity = user.result.attrs[k].value;
    }
    console.log ("Result after set",user.result);
