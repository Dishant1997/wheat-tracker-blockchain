...
async function publicSubmitTx(txName, ...args) {
    try {
        //  check header; get username and pwd from request
        //  does NOT verify auth credentials
        return utils.setUserContext("admin", "admin").then((contract) => {
            // Insert txName as args[0]
            args.unshift(txName);
            // Insert contract as args[0]
            args.unshift(contract);
...
supplychainRouter.route('/customer-orders').get(function (request, response) {
    submitTx(request, 'queryAllCustomerOrders', '')
        .then((queryCustomerOrderResponse) => {
            //  response is already a string;  not a buffer
            console.log("Response", queryCustomerOrderResponse)
            let orders = queryCustomerOrderResponse;
            response.status(STATUS_SUCCESS);
            response.send(orders);
        }, (error) => {
            response.status(STATUS_SERVER_ERROR);
...
supplychainRouter.route('/public-orders').get(function (request, response) {
    publicSubmitTx('queryAllOrders', '')
        .then((queryOrderResponse) => {
            //  response is already a string;  not a buffer
            let orders = queryOrderResponse;
            response.status(STATUS_SUCCESS);
...
supplychainRouter.route('/customer-orders/:id').put(function (request, response) {
    submitTx(request, 'customerOrder', request.params.id)
        .then((customerOrderResponse) => {
            ...
            let order = Order.fromBuffer(customerOrderResponse);
            console.log(`order ${order.orderId} : state = ${order.currentOrderState} : customer = ${order.customerId}`);
            response.status(STATUS_SUCCESS);
            response.send(order);
        }, (error) => {
            response.status(STATUS_SERVER_ERROR);
...
supplychainRouter.route('/public-orders/:id').get(function (request, response) {
    publicSubmitTx('getOrderHistory', request.params.id)
        .then((queryOrderResponse) => {
            // process response
            //let order = Order.fromBuffer(queryOrderResponse);
            //console.log(`order ${order.orderId} : price = ${order.price}, quantity = ${order.quantity}, producer = ${order.producerId}, consumer = ${order.retailerId}, trackingInfo = ${order.trackingInfo}, state = ${order.currentOrderState}`);
            response.status(STATUS_SUCCESS);
            response.send(queryOrderResponse);
        }, (error) => {
            response.status(STATUS_SERVER_ERROR);
            response.send(utils.prepareErrorResponse(error, STATUS_SERVER_ERROR,
...
// Change status to ORDER_RECEIVED
supplychainRouter.route('/receive-customer-order/:id').put(function (request, response) {
    submitTx(request, 'receiveCustomerOrder', request.params.id)
        .then((receiveOrderResponse) => {
            // process response
            console.log('Process ReceiveOrder transaction.');
            let order = Order.fromBuffer(receiveOrderResponse);
...
supplychainRouter.route('/create-customer-shipment-for-order/:id').put(function (request, response) {
    submitTx (request, 'createCustomerShipment', request.params.id, utils.getRandomNum())
      .then((createShipmentResponse) => {
          console.log('Process CreateShipment transaction.');
          let order = Order.fromBuffer(createShipmentResponse);
          console.log(`order ${order.orderId} : trackingInfo = ${order.trackingInfo}, state = ${order.currentOrderState}`);
...
// This changes the status on the order
supplychainRouter.route('/transport-customer-shipment/:id').put(function (request, response) {
    submitTx(request, 'transportCustomerShipment', request.params.id)
        .then((transportShipmentResponse) => {
            console.log('Process TransportShipment transaction.');
            let order = Order.fromBuffer(transportShipmentResponse);
            console.log(`order ${order.orderId} : state = ${order.currentOrderState}`);
...
supplychainRouter.route('/receive-customer-shipment/:id').put(function (request, response) {
    submitTx(request, 'receiveCustomerShipment', request.params.id)
        .then((receiveShipmentResponse) => {
            // process response
            console.log('Process ReceiveShipment transaction.');
            let order = Order.fromBuffer(receiveShipmentResponse);
...
supplychainRouter.route('/sign-up-user').post(function (request, response) {
    try {
        let userId = request.body.userid;
        let userPwd = request.body.password;
        let userType = request.body.usertype;
        let userCnic = request.body.cnic;
        ...
        //  So, it is assumed that only the admin has access to this api
        //  register-user can only be called by a user with admin privileges.
        utils.registerUser(userId, userPwd, userType, userCnic, userCity, "admin").
            then((result) => {
                console.log(result)
