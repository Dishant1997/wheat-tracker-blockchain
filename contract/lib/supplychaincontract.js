...
class SupplychainContract extends Contract {
    ...
    */
    ...
    async receiveOrder(ctx, orderId) {
      ...
      * Usage:  receiveOrder ('Order001')
     */
    async receiveCustomerOrder(ctx, orderId) {
        console.info('============= receiveOrder ===========');
        let userCity = await this.getCurrentUserCity(ctx);
        ...
        // Change currentOrderState
        order.setStateToCustomerOrderReceived();

        // Track who is invoking this transaction
        order.modifiedBy = userId;
        order.currentState = "CUSTOMER_ORDER_RECEIVED";
        order.currentOrderState = 9;
        order.city = userCity;
   ...
   async assignCustomerShipper(ctx, orderId, newShipperId) {
    ...
    // Change currentOrderState to SHIPMENT_ASSIGNED;
    order.setStateToCustomerShipmentAssigned();
    // Set shipperId
    order.shipperId = newShipperId;
    // Track who is invoking this transaction
    order.modifiedBy = userId;
    ...
    */
    async createShipment(ctx, orderId, newTrackingInfo) {
        ...
        // Track who is invoking this transaction
        order.modifiedBy = userId;
        order.currentState = "SHIPMENT_CREATED";
        order.currentOrderState = 4;
        order.city = userCity;
     ...
     * Usage:  createShipment ('Order001', '34590279RKE9D339')
    */
   async createCustomerShipment(ctx, orderId, newTrackingInfo) {
    console.info('============= createShipment ===========');
    let userCity = await this.getCurrentUserCity(ctx);
    ...
    // Change currentOrderState to SHIPMENT_CREATED;
    order.setStateToCustomerShipmentCreated();
    // Set Tracking info
    order.trackingInfo = newTrackingInfo;
    // Track who is invoking this transaction
    order.modifiedBy = userId;
    order.currentState = "CUSTOMER_SHIPMENT_CREATED";
    order.currentOrderState = 11;
    order.city = userCity;
    // Update ledger
    await ctx.stub.putState(orderId, order.toBuffer());
    ...
    */
    async transportShipment(ctx, orderId) {
        ...
        // Track who is invoking this transaction
        order.modifiedBy = userId;
        order.currentState = "SHIPMENT_IN_TRANSIT";
        order.currentOrderState = 5;
        order.city = userCity;
        // Update ledger
        await ctx.stub.putState(orderId, order.toBuffer());
     ...
     * Usage:  transportShipment ('Order001')
    */
   async transportCustomerShipment(ctx, orderId) {
    console.info('============= transportShipment ===========');
    let userCity = await this.getCurrentUserCity(ctx);
    ...
    // Change currentOrderState to SHIPMENT_IN_TRANSIT;
    order.setStateToCustomerShipmentInTransit();
    // Track who is invoking this transaction
    order.modifiedBy = userId;
    order.currentState = "CUSTOMER_SHIPMENT_IN_TRANSIT";
    order.currentOrderState = 12;
    order.city = userCity;
    ...
    */
    async receiveShipment(ctx, orderId) {
        ...
        // Track who is invoking this transaction
        order.modifiedBy = userId;
        order.currentState = "SHIPMENT_RECEIVED";
        order.currentOrderState = 6;
        order.city = userCity;
     ...
     * Usage:  receiveShipment ('Order001')
    */
   async receiveCustomerShipment(ctx, orderId) {
    console.info('============= receiveCustomerShipment ===========');
    let userCity = await this.getCurrentUserCity(ctx);
    ...
    if ((userId != "admin") // admin only has access as a precaution.
        && (userId != order.customerId)) // This transaction should only be invoked by
        throw new Error(`${userId} does not have access to receive shipment for order ${orderId}`);

    // Change currentOrderState to SHIPMENT_RECEIVED;
    order.setStateToCustomerShipmentReceived();
    // Track who is invoking this transaction
    order.modifiedBy = userId;
    order.currentState = "CUSTOMER_SHIPMENT_RECEIVED";
    order.currentOrderState = 13;
    order.city = userCity;
    // Update ledger
    await ctx.stub.putState(orderId, order.toBuffer());
    ...
    async customerOrder(ctx, orderId) {
        ...
        var orderAsBytes = await ctx.stub.getState(orderId);
        if (!orderAsBytes || orderAsBytes.length === 0) {
            throw new Error(`Error Message from customerOrder: Order with orderId = ${orderId} does not exist.`);
        }
        ...
        if ((userId != "admin") // admin only has access as a precaution.
            && (userType != "customer")) // This transaction should only be invoked by
            throw new Error(`${userId} does not have access to receive shipment for order ${orderId}`);

        // Change currentOrderState to SHIPMENT_RECEIVED;
        order.setStateToCustomerOrderCreated();
        // Track who is invoking this transaction
    ...
    */
    ...
    async queryAllCustomerOrders(ctx) {
        console.info('============= getCustomerOrderHistory ===========');
        ...
        console.log("In queryAllCustomerOrders: queryString = ");
        console.log(queryString);
        // Get all orders that meet queryString criteria
        const iterator = await ctx.stub.getQueryResult(JSON.stringify(queryString));
        const allOrders = [];
    ...
    async getCurrentUserId(ctx) {
        console.log("CTXXXXXXXXXXXXXXXX",ctx)
        let id = [];
    ...
    async getCurrentUserCity(ctx) {

        return ctx.clientIdentity.getAttributeValue("userCity");
    }
